...
export function formulaSystemPrompt(variablesContext: string) {
  return `You are a KPI formula assistant. The user describes a calculation in plain language (Arabic or English).
...
KPI Glossary (use these exact translations):
...
export const describeSystemPrompt = `You are a KPI documentation specialist. Given a KPI title, generate a professional description in both English and Arabic.
...
- Focus on what the KPI measures, how it's typically collected, and why it matters
...
export function suggestNoteSystemPrompt(locale: string) {
  return locale === "ar"
    ...
    : `You are an assistant helping KPI managers write explanatory notes. Write a short professional note (1-2 sentences) explaining the deviation of the entered value from the historical average. Be objective and professional. Do not invent specific reasons — suggest general possible causes.`;
}
...
- أنهِ التقرير بتوصيات عملية`
    : `You are a strategic performance analyst for the "Rafed" KPI management platform. Write a professional performance report based ONLY on the approved data provided.
