...
export default function ProjectDetailPage() {
  ...
  const [project, setProject] = useState<EntityDetail | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    getOrgEntityDetail({ entityId: params.projectId })
      .then(setProject)
        ...
        <TabsContent value="summary" className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="border-border bg-card/50 shadow-sm">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              >
                <p className="font-semibold text-foreground">{t("backToProjects")}</p>
              </Link>
            </CardContent>
          </Card>
        </TabsContent>
