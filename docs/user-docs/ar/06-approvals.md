...
```mermaid
stateDiagram-v2
    [*] --> PENDING : إرسال القيمة
    
    PENDING --> UNDER_REVIEW : مراجعة المعتمد
    UNDER_REVIEW --> APPROVED : قرار بالاعتماد
    UNDER_REVIEW --> REJECTED : قرار بالرفض
    
    REJECTED --> PENDING : إعادة الإرسال
...
```mermaid
flowchart TD
    A[قيمة مُرسَلة] --> B{المعتمد يراجع}
    
    B -->|القيمة صحيحة| C{قرار الاعتماد}
    B -->|القيمة خاطئة| D{قرار الرفض}
    
    C -->|✅ اعتماد| E[تحديث الحالة إلى APPROVED]
    C -->|⏳ تأجيل| F[البقاء في الانتظار]
    
    D -->|❌ رفض مع سبب| G[إضافة تعليق]
    D -->|📝 رفض للتعديل| H[إعادة للمسودة]
    
    G --> I[إشعار المُرسل بالرفض]
    H --> I
    
    E --> J[إشعار المُرسل بالاعتماد]
    
    I --> K[المستخدم يُعيد الإدخال]
    K --> L[تصحيح القيمة]
    L --> M[إعادة الإرسال]
    M --> A
    
    J --> N[القيمة معتمدة]
    F --> O[الانتظار لمراجعة لاحقة]
```
